"""Applies the diagnostic's spectral throughput/sensitivity correction (from the transmission function of
the optics chain) to raw data, per Thomson scattering type and shot number."""
import xlrd
import numpy as np
...
def correctThroughput(data, tstype, axisy, shotNum):
    ...
    if tstype == "angular":
        ...
        if np.mean(axisy) <425.0:
            ...
            sens[:, 0] = sheet.col_values(0)[2:303]
            sens[:, 1] = sheet.col_values(1)[2:303]
