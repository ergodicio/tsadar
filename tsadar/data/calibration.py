"""Instrument calibration lookups: probe-beam scattering angles/weights (sa_lookup) and the wavelength,
timing, and instrument-response calibrations for a given shot/diagnostic type (get_calibrations)."""
...
def sa_lookup(beam):
    ...
    beam. All values are precalculated. Available options are P9, B12, B15, B23, B26, B35, B42, B46, B58, B62.
    ...
    """
    try:
        sa_start, sa_end, weights = _SCATTERING_ANGLES[beam]
    except KeyError:
        raise NotImplementedError("Other probe geometrries are not yet supported")

    return dict(sa=np.linspace(sa_start, sa_end, 10), weights=np.array(weights))
