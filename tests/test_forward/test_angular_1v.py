...
def test_arts1d_forward_pass():
    ...
    with mlflow.start_run(run_name="test_arts1v_fwd") as run:
        ...
        [axisxE, _, _, _, _, _] = get_calibrations(
            104000, config["other"]["extraoptions"]["spectype"], 0.0, config["other"]["CCDsize"], config["other"]["detector_specs"],
        ...
        # np.save("tests/test_forward/ThryE-arts1v.npy", ThryE)

        ground_truth = np.load("tests/test_forward/ThryE-arts1v.npy")
