...
def test_arts2d_inverse(arbitrary_distribution: bool):
    """
    ...
    with mlflow.start_run(run_name="test_arts2d_inverse") as run:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            # mlflow.log_metric(f"first run time", first_run_time)
            ...
            while np.nan_to_num(loss, nan=1) > 5e-2:
                ...
                if use_optax:
                    ...
                    for i in (pbar := tqdm.tqdm(range(25))):
                        ...
                        grad_norm = np.linalg.norm(ravel_pytree(grad_loss)[0])
                        # mlflow.log_metric(f"iteration time", time.time() - t0, step=i)
                        mlflow.log_metrics(
                            {"loss": loss, "grad_norm": grad_norm, "iteration_time": time.time() - t0}, step=i
                        )

                        updates, opt_state = opt.update(grad_loss, opt_state)
                        diff_params = eqx.apply_updates(diff_params, updates)
                        pbar.set_description(f"Loss: {loss:.2e}, Grad Norm: {grad_norm:.2e}")
...
if __name__ == "__main__":
    test_arts2d_inverse(arbitrary_distribution=False)
